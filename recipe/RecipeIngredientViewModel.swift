...
class RecipeIngredientViewModel: ObservableObject {
    ...
    @Published var recipes: [Recipe] = []{
        didSet {
            // No need for filteredRecipes now
        }
    }  // Updated to `recipes` to match usage in views
    ...
    @Published var editingRecipe: Recipe?
        ...
        //MARK: Function to add the ingredient
        func addIngredient() {
    ...
    //MARK: Function to initialize editing for an existing recipe
    ...
    //MARK: Add the updateRecipe function to update an existing recipe
    func updateRecipe() {
    ...
    func deleteRecipe(recipe: Recipe) {
     ...
     //MARK: function takes a MeasurementType as an argument and sets measurement to the new value.
    ...
    //MARK: function that increment the serving
     func incrementServing() {
         if servingSize < 20 {
             servingSize += 1
         }
     }
     //MARK: function tha  decrement the serving
    ...
    //MARK: This method clears any values from the new ingredient form so that the form is reset for the next entry.
    ...
    //MARK: This method clears any values from the new recipe form so that the form is reset for the next entry.
